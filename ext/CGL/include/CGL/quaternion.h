...
class Quaternion : public Vector4D {
 public:
   ...
   */
  Quaternion(const Vector3D& v, long double w) : Vector4D(v.x, v.y, v.z, w) { }

  Quaternion(const Vector4D& v) : Vector4D(v.x, v.y, v.z, v.w) { }

  Quaternion(long double x, long double y, long double z, long double w) : Vector4D(x, y, z, w) { }

  /**
   * Initializes a quaternion that represents a rotation about the given axis
   * and angle.
   */
  void from_axis_angle(const Vector3D& axis, long double radians) {
    radians /= 2;
    const Vector3D& nAxis = axis.unit();
    long double sinTheta = sin(radians);
    x = sinTheta * nAxis.x;
    y = sinTheta * nAxis.y;
    z = sinTheta * nAxis.z;
    w = cos(radians);
  ...
  long double real() const { return w; }
  void setReal(long double r) { w = r; }
  ...
  Vector3D scaledAxis(void) const{
	...
	long double angle = 2 * acos(q1.w);

	// s must be positive, because q1 <= 1, due to normalization.
	long double s = sqrt(1-q1.w*q1.w);
  ...
  {
	long double theta = vec_in.norm();

	// Small magnitudes are handled via the default vector.
	if (theta > 0.0001)
	{
	  long double s = sin(theta / 2.0);
  ...
  void euler(const Vector3D& euler) {
	long double c1 = cos(euler[2] * 0.5);
	long double c2 = cos(euler[1] * 0.5);
	long double c3 = cos(euler[0] * 0.5);
	long double s1 = sin(euler[2] * 0.5);
	long double s2 = sin(euler[1] * 0.5);
	long double s3 = sin(euler[0] * 0.5);
  ...
  {
	Vector3D euler;
	const static long double PI_OVER_2 = M_PI * 0.5;
	long double sqw, sqx, sqy, sqz;
  ...
  void decoupleZ(Quaternion* Qxy, Quaternion* Qz) const {
	...
	long double axis_norm = axis_xy.norm();

	long double axis_theta = acos(clamp(zbt.z, -1.0l, 1.0l));
   ...
   */
  Quaternion slerp(const Quaternion& q1, long double t)
  {
    return slerp(*this, q1, t);
  }

  /// Returns quaternion that is slerped by fraction 't' between q0 and q1.
  static Quaternion slerp(const Quaternion& q0, const Quaternion& q1, long double t) {

    long double omega = acos(clamp(q0.x*q1.x +
                              q0.y*q1.y +
                              q0.z*q1.z +
                              q0.w*q1.w,  -1.0l, 1.0l));
    if (fabs(omega) < 1e-10)
    ...
    long double som = sin(omega);
    long double st0 = sin((1-t) * omega) / som;
    long double st1 = sin(t * omega) / som;
 ...
 */
Quaternion operator*(long double s, const Quaternion& q);
