...
void DrawRend::rasterize_triangle_temp( float x0, float y0,
                         float x1, float y1,
                         float x2, float y2,
  ...
  // Determine bounding box coords
  float xLow = min(min(x0, x1), x2);
  float xHigh = max(max(x0, x1), x2);
  float yLow = min(min(y0, y1), y2);
  float yHigh = max(max(y0, y1), y2);

  // Iterate through bounding box
  for (int i = (int)yLow; i < yHigh; i++) {
    for (int j = (int)xLow; j < xHigh; j++) {
      float y = i + .5, x = j + .5;
      // Check that point is on screen
      if (!in_bounds(x, y)) continue;

      // Check that point is inside triangle
      if (side_check(x0, y0, x1, y1, x2, y2, x, y) < 0) continue;
      if (side_check(x1, y1, x2, y2, x0, y0, x, y) < 0) continue;
      if (side_check(x2, y2, x0, y0, x1, y1, x, y) < 0) continue;
...
void DrawRend::rasterize_triangle( float x0, float y0,
                         ...
                         float x2, float y2,
                         Color color, Triangle *tri) {
  // Pre-computations for bounding
  float x01 = x1 - x0, y01 = y1 - y0;
  float x12 = x2 - x1, y12 = y2 - y1;
  float x20 = x0 - x2, y20 = y0 - y2;
  float c01 = (x01 * -y20) - (y01 * -x20);
  float c12 = (x12 * -y01) - (y12 * -x01);
  float c20 = (x20 * -y12) - (y20 * -x12);
  ...
  for (int i = (int)yLow; i < yHigh; i++) {
    for (int j = (int)xLow; j < xHigh; j++) {
      ...
      int ss = samplebuffer[i][j].samples_per_side;
      float gap = 1.0 / ss;
      float edgeGap = gap / 2.0;
      for (int k = 0; k < ss; k++) {
        for (int l = 0; l < ss; l++) {
          float y = i + k*gap + edgeGap, x = j + l*gap + edgeGap;

          // Check that point is on screen
          if (!in_bounds(x, y)) continue;

          // Check that point is inside triangle
          if (side_check_fast(c01, x01, y01, x-x0, y-y0) < 0) continue;
          if (side_check_fast(c12, x12, y12, x-x1, y-y1) < 0) continue;
          if (side_check_fast(c20, x20, y20, x-x2, y-y2) < 0) continue;

          // Tri behaviour
          if (tri != nullptr) {
            // Compute barycentric coordinates 
            Vector3D p_bary = barycentric(x, y, x0, y0, x1, y1, x2, y2);
            Vector3D p_dx_bary = barycentric(x+1, y, x0, y0, x1, y1, x2, y2);
            Vector3D p_dy_bary = barycentric(x, y+1, x0, y0, x1, y1, x2, y2);

            // SampleParams
            SampleParams sp;
            sp.psm = psm;
            sp.lsm = lsm;
            samplebuffer[i][j].fill_color(k, l, tri->color(p_bary, p_dx_bary, p_dy_bary, sp));
          } else {
            samplebuffer[i][j].fill_color(k, l, color);
          }
        }
      }
    }
...
// Checks if a given (x, y) point is within samplebuffer's bounds.
bool DrawRend::in_bounds(float x, float y) {
  return 0 <= y && y < samplebuffer.size() && 0 <= x && x < samplebuffer[0].size();
}

// Checks if C and X are on the same side as the lined formed from A and B.
// if > 0, they're on the same side.
// if < 0, they're on different sides.
// if = 0, X is on line AB.
float DrawRend::side_check(float xA, float yA, float xB, float yB,
                           float xC, float yC, float xX, float yX) {
  float xBA = xB - xA, yBA = yB - yA;
  float xCA = xC - xA, yCA = yC - yA;
  float xXA = xX - xA, yXA = yX - yA;
  float cBC = xBA * yCA - yBA * xCA;
  float cBX = xBA * yXA - yBA * xXA;
  return cBC * cBX;
}

// Faster version of side_check. V is base vector, U is new vector.
float DrawRend::side_check_fast(float cross, float xV, float yV,
                                float xU, float yU) {
  float newCross = xV * yU - yV * xU;
  return cross * newCross;
}

// Compute Barycentric coordinates
Vector3D DrawRend::barycentric(float x, float y,
                     float x0, float y0,
                     float x1, float y1,
                     float x2, float y2) {
  // Pre-compute values
  double yCB, xCB, yAC, xAC, den0, den1;
  yCB = y2 - y1; xCB = x2 - x1;
  yAC = y0 - y2; xAC = x0 - x2;
  den0 = -(x0 - x1) * yCB + (y0 - y1) * xCB;
  den1 = -(x1 - x2) * yAC + (y1 - y2) * xAC;

  double alpha = (-(x-x1)*yCB + (y-y1)*xCB) / den0;
  double beta = (-(x-x2)*yAC + (y-y2)*xAC) / den1;
