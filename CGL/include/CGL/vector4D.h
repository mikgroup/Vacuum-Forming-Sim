...
class Vector4D {
 ...
 Vector4D() : x( 0.0 ), y( 0.0 ), z( 0.0 ), w( 0.0 ) { }
   ...
   */
 Vector4D( long double x, long double y, long double z, long double w) : x( x ), y( y ), z( z ), w( w ) { }

  /**
   * Constructor.
   * Initializes to vector (x,y,z,0).
   */
 Vector4D( long double x, long double y, long double z) : x( x ), y( y ), z( z ), w( 0.0 ) { }
   ...
   */
 Vector4D( long double c ) : x( c ), y( c ), z( c ), w ( c ) { }
 ...
 /**
   ...
   */
 Vector4D( const Vector3D& v, long double w ) : x( v.x ), y( v.y ), z( v.z ), w( w ) { }
  ...
  inline Vector4D operator*( const long double& c ) const {
  ...
  inline Vector4D unit( void ) const {
    long double rNorm = 1. / sqrt( x*x + y*y + z*z + w*w);
...
inline Vector4D operator* ( const long double& c, const Vector4D& v ) {
  return Vector4D( c * v.x, c * v.y, c * v.z, c*v.w );
}

// dot product (a.k.a. inner or scalar product)
inline long double dot( const Vector4D& u, const Vector4D& v ) {
