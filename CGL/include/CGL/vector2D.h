...
class Vector2D {
 public:
  ...
  inline Vector2D operator*( long double r ) const {
    Vector2D vr = *this;
    vr *= r;
    return vr;
  }
  ...
  inline void operator*=( long double r ) {
    x *= r;
    y *= r;
  }
...
inline Vector2D operator*( long double r, const Vector2D& v ) {
   return v*r;
}

// inner product
inline long double dot( const Vector2D& v1, const Vector2D& v2 ) {
  return v1.x*v2.x + v1.y*v2.y;
}

// cross product
inline long double cross( const Vector2D& v1, const Vector2D& v2 ) {
