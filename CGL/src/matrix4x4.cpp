...
namespace CGL {
  ...
  long double& Matrix4x4::operator()( int i, int j ) {
    return entries[j][i];
  }

  const long double& Matrix4x4::operator()( int i, int j ) const {
    return entries[j][i];
  }
  ...
  void Matrix4x4::zero( long double val ) {
  ...
  long double Matrix4x4::det( void ) const {
  ...
  long double Matrix4x4::norm( void ) const {
  ...
  void Matrix4x4::operator+=( const Matrix4x4& B ) {
    ...
    Matrix4x4& A( *this );
    long double* Aij = (long double*) &A;
    const long double* Bij = (const long double*) &B;

	// Add the 16 contigous vector packed long double values.
    *Aij++ += *Bij++;//0
    *Aij++ += *Bij++;
    *Aij++ += *Bij++;
    *Aij++ += *Bij++;
  ...
  Matrix4x4 Matrix4x4::operator*( long double c ) const {
    const Matrix4x4& A( *this );
    Matrix4x4 B;
  ...
  // Returns c*A.
  Matrix4x4 operator*( long double c, const Matrix4x4& A ) {

    Matrix4x4 cA;
    const long double* Aij = (const long double*) &A;
    long double* cAij = (long double*) &cA;
  ...
  void Matrix4x4::operator/=( long double x ) {
    Matrix4x4& A( *this );
    long double rx = 1./x;
