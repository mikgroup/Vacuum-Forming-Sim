...
namespace CGL {
  ...
  long double& Matrix3x3::operator()( int i, int j ) {
    return entries[j][i];
  }

  const long double& Matrix3x3::operator()( int i, int j ) const {
    return entries[j][i];
  }
  ...
  void Matrix3x3::zero( long double val ) {
    // sets all elements to val
    entries[0] = entries[1] = entries[2] = Vector3D( val, val, val );
  }

  long double Matrix3x3::det( void ) const {
  ...
  long double Matrix3x3::norm( void ) const {
  ...
  Matrix3x3 Matrix3x3::operator*( long double c ) const {
    const Matrix3x3& A( *this );
    Matrix3x3 B;
  ...
  void Matrix3x3::operator/=( long double x ) {
    Matrix3x3& A( *this );
    long double rx = 1./x;
